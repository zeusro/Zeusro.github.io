...
Лишь в 2019 году, в версии Go 1.13, появились errors.Is и errors.As. До этого (и нередко до сих пор) разработчикам приходилось создавать собственные типы ошибок через реализацию интерфейсов или использовать приведение типов для определения конкретного вида ошибки.

По сравнению с моделью исключений в Java, где функция может явно throw исключение, такой подход выглядит довольно расплывчатым. В golang возможные ошибки выражаются через множественные возвращаемые значения, где error обычно является последним результатом функции.

Однако такое представление не всегда удобно для вызывающей стороны. Она всегда получает лишь объект типа error, но не знает, с каким именно видом ошибки имеет дело. Чтобы определить причину сбоя, приходится дополнительно использовать errors.Is、errors.As、проверки интерфейсов или приведение типов.
