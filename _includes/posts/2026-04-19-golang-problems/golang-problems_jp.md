...
Go は 2019 年の Go 1.13 になってようやく errors.Is と errors.As を導入した。それまでも現在でも、独自のエラー型を扱うには、インターフェース実装や型アサーションによってエラー種別を判定することが多い。

これは Java の throw / catch による例外機構と比べると、やや曖昧に感じられる。golang では関数の失敗可能性を、多値返却の最後の戻り値である error によって表現する。

しかし呼び出し側から見ると、この表現は必ずしも明確ではない。受け取れるのは常に error であり、その具体的な型や原因を知るには、追加の判定やラッピング、型アサーションが必要になる。
