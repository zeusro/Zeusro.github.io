...
go直到2019年才在1.13中使用推出 errors.Is 和 errors.As ，用户需要通过实现接口或者类型断言的方式来“制造”自定义类型的error。

这种方式比起Java那种 function throw exception ，就显得很模糊。在golang方法的声明中，是通过多返回值（error作为最后一个返回）的形式来表达可能出现的异常。

但这种表达是很模糊的，对于调用方来说，永远只看到一个 error ，但他不知道具体是什么样类型的错误。
