...
**Учитель F**: использует формулы 1 и 2. Число учеников в формуле 1 не меняет, но может уменьшать число сдающих экзамен ради доли поступления.

**Учитель D**: использует формулы 1 и 2. Максимизирует среднюю успеваемость без исключения учеников.
...
- **Визуализация**: стимул сэмплируется как `(t, результативность)`; время по оси x.
...
Результативность (воспринимаемый руководством стимул) изоморфна выигрышу учителя как функция времени \(t\):
...
При заданных выигрышах и пространстве стратегий равновесие и рекомендации (модель и теория совпадают):
...
- **Выход**: (1) временной лог; (2) сэмпл стимула (время → результативность); (3) итоговая статистика (в школе, сдающие, поступившие, средняя успеваемость, доля, результативность).
